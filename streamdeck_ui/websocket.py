...
from streamdeck_ui import __version__
...
class StreamDeckWsServer(QObject):
    ...
    def __init__(self, port: int, parent=None) -> None:
        ...
        #self.host = PySide6.QtNetwork.QHostAddress.LocalHost
        self.host = PySide6.QtNetwork.QHostAddress.AnyIPv4
        self.port = port
        self.clients = []
...
class PluginServer(QObject):
    ...
    def __key_id_from_column_row(self, deck_id, column: int, row: int):
        device_cols = api.decks_by_serial[deck_id].KEY_COLS
    ...
    def receive_event(self, client: QWebSocket, evt: Dict[str, Any]):
        ...
        try:
            if event_name == 'setPage':
                ...
                if page_num is not None and page_num >= 0 and device_id in api.decks_by_serial:
    ...
    def handle_keypress(self, deck_id: str, key: int, state: bool) -> None:
        deck = api.decks_by_serial[deck_id]
    ...
    def on_client_connected(self, client: QWebSocket):
        ...
        devices = []
        for deck_id, deck in api.decks_by_serial.items():
            devices.append({
                'id': deck_id,
                'size': {
